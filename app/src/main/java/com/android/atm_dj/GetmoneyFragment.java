     ...
     * {@link GetmoneyFragment.OnFragmentInteractionListener} interface
    ...
    public class GetmoneyFragment extends Fragment {
        ...
        private String str,str_nowmoney,str_remoney,str_username,str_paswd;
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
            ...
            View view = inflater.inflate(R.layout.fragment_getmoney, container, false);
            editText = view.findViewById(R.id.et_getmoeny);
            button = view.findViewById(R.id.frag_getmoney);
            button.setOnClickListener(new Button.OnClickListener() {
                ...
                if("".equals(str_money)){
                    ...
                    if (i_money <= 5000 && i_money % 100 == 0 && i_money>0) {
                        ...
                        if (i_nowmoney - i_money >= 0) {
                            sqLiteDatabase_update = us_wri.getWritableDatabase();
                            str_remoney = String.valueOf(i_nowmoney - i_money);
    //                        Toast.makeText(button.getContext(),int_id+" "+str_username+" "+str_paswd+" "+str_remoney , Toast.LENGTH_SHORT).show();
                            us_wri.update(sqLiteDatabase_update,int_id,str_username,str_paswd,str_remoney);
                            Toast.makeText(button.getContext(), "取款成功！", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(button.getContext(), "您的账户余额不足！", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(button.getContext(), "每次只能取整且不能超过5000元！", Toast.LENGTH_SHORT).show();
                    }
                }
                }
            });
